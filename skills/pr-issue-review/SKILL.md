...
- Grade the merged state on its own, applying the de-escalations in Finding Severity, and let the label speak only after that. A deferred item that leaves the merged surface with a live auth gap, a tenant-isolation hole, a data-loss or silent-failure path, or an unguarded read-then-write race is a `⚠️ P1` however it is labelled. Keep it, and say explicitly: "the PR body defers this, but the merged state is unsafe until the follow-up lands." A tracked reference does not clear a P1.
- Deferred enforcement is the case most often over-graded here, so grade it before the label reaches you. A restriction the client enforces while the server accepts a write from a caller it should have rejected is a `⚠️ P1`. A restriction the client enforces on top of a server check that already authorizes this caller for this resource is not: bypassing it gains the caller nothing they did not already hold, so it grades by blast radius like anything else and lands at `💭 P4` or `ℹ️ FYI` (see Finding Severity).
...
- A gap in a permission gate grades by what bypassing the gate gains the caller beyond the access they already hold, so establish the server-side check that does run before grading the one that does not. When every caller who can reach the write already holds write access to that resource, and the bypass only changes their own tenant's presentation, preference, or configuration state, it is at most `💭 P4`, and `ℹ️ FYI` when the PR body states the gate is a product affordance rather than a security boundary. Unlocking a staff-gated control over data the caller already administers is not escalation for this test. Reaching another tenant's data, an unauthenticated path, an action the caller's role could never perform, exposure of data to someone who could not already see it, money movement, and destructive or irreversible writes all keep their full severity.
